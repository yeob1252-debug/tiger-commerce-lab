...
- 검사 대상 원격 커밋: `47259a9db46b3a5c8e12fe5a88edb1fb9aed5186`
- Vercel Preview 배포: `dpl_Ch6fiNe2wTongEtKhV1yfw1yva6z` (`READY`)
...
- 추가 생성 이미지: 호랑이 밝은 레이어 144KB / 맘커뮤니티 확산 장면 88KB
...
| HERO | 동일 구도 원본·밝은 레이어 2장, 포인터 근접 시 레이어 opacity 0→0.793, 순환문구 정상 |
| HERO 눈빛 | 가짜 조명 DOM 0개, 원본 이미지의 실제 눈·얼굴만 밝아짐 |
| 두 번째 후킹 | “고객은 이미 SNS에 있습니다. 우리 매장만 안 보일 뿐입니다.” 표시 |
| 이용 통계 | 2시간 32분·34분·94.7% 카운트 애니메이션 후 최종값 정확 |
...
| 맘커뮤니티 이미지 | 1587×991 실사형 장면 로드, `실제 후기·성과값 아님` 고지 표시 |
| 66.1K | 부모·지표행 transform `none`, 숫자만 애니메이션, K 접미사 간격 12px |
| LIVE 고지 | 직접 방송·대행 방송·PG/배송비 조건 정확히 구분 |
| FAQ | 단일 항목 확장 및 답변 정상 |
| Kakao CTA | `https://open.kakao.com/o/sgxBgDIi` 2곳 연결 |
| 이미지 | 깨진 이미지 0 |
| 브라우저 오류 | 홈페이지 코드 오류 0 (브라우저 확장 자체 로그 제외) |
