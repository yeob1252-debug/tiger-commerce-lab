(() => {
  ...
  const HERO_SLIDES = [
    {
      ...
      titleHTML: '장사는 <span class="hl">사장님</span>이,<br>고객이 오는 <span class="hl">채널</span>은 타이거가 운영합니다.',
    ...
    },
    {
      ...
      titleHTML: '배달 반경은 짧아도,<br><span class="hl">메뉴가 팔릴 범위</span>까지 짧을 필요는 없습니다.',
    ...
    },
    {
      ...
      titleHTML: '손님은 매장에 오기 전,<br><span class="hl">SNS</span>에서 이미 어디로 갈지 정합니다.',
    ...
    },
    {
      ...
      titleHTML: '검색됐는데 <span class="hl">선택받지 못하면</span>,<br>좋은 메뉴도 발견되지 않습니다.',
    ...
    },
    {
      ...
      titleHTML: '게시물 몇 개가 아니라,<br>고객이 <span class="hl">다시 찾아오는 구조</span>를 만듭니다.',
  ...
  function renderHeroSlide(i, { announce = true } = {}) {
    ...
    heroTitleEl.innerHTML = slide.titleHTML;
  ...
  function selectDiagCard(type) {
    ...
    ALL_FIELDSET_IDS.forEach((id) => {
      const el = document.getElementById(id);
      if (el) el.hidden = id !== cfg.fieldsetId;
    });
  ...
  function showPlanPanel() {
    ...
    ALL_FIELDSET_IDS.forEach((id) => { const el = document.getElementById(id); if (el) el.hidden = true; });
  ...
  let heroPassed = false, footerVisible = false, formFocused = false;
  function updateMobileCtaVisibility() {
    if (!mobileFixedCta) return;
    mobileFixedCta.classList.toggle('is-hidden', !(heroPassed && !footerVisible && !formFocused));
  }
  ...
  if (dashTabs.length && dashShotImg) {
    dashTabs.forEach((tab) => {
      tab.addEventListener('click', () => {
        ...
        if (shot) { dashShotImg.src = shot.src; dashShotImg.alt = shot.alt; }
      });
    });
  }
  ...
  if (planTermToggle && planSummaryGrid && planDetail) {
    ...
    function renderPlanDetail(planId) {
      ...
      if (ctaBtn) ctaBtn.addEventListener('click', () => {
        openDiagnosisPanel({ type: 'plan', plan: p.id, term: planState.term, sourceSection: 'service-plan', sourceCTA: 'plan-cta-' + p.id });
      });
    }
