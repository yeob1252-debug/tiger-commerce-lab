...
(async () => {
  ...
  assert.match(css, /\.hero-media\s*\{\s*position:\s*relative;\s*inset:\s*auto;\s*height:\s*138svh/);
  assert.match(css, /\.story\s*\{\s*min-height:\s*250svh/);
  assert.match(css, /\.story-sticky[^}]*grid-template-columns:\s*minmax\(0,1fr\)/);
  assert.match(css, /\.process\s*\{\s*min-height:\s*250svh/);
  assert.match(css, /\.operation-showcase\s*\{\s*min-height:\s*205svh/);
  ...
  assert.ok(fs.existsSync(path.join(repo, 'assets/home/v7/tiger-hero-mobile-cinematic.webp')));
  assert.ok(fs.existsSync(path.join(repo, 'assets/home/v7/tiger-hero-mobile-illuminated.webp')));
